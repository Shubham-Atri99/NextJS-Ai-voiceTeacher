...
const ProfilePage = async () => {
  ...
  if (!user) {
    return <main className="p-4">Please sign in to view your profile.</main>;
  }

  const sessionsCompleted = await getrecentSessions();
    ...
    <main className="lg:w-3/4 w-full mx-auto p-4">
      <section className="flex justify-between items-center max-sm:flex-col max-sm:items-start gap-6">
        ...
        <div className="flex gap-6">
          ...
          <div className="flex flex-col items-center px-6 py-4 border border-gray-300 rounded-2xl shadow-sm bg-white w-48">
            ...
            <div className="text-2xl font-bold text-gray-800">{sessionsCompleted.length}</div>
      ...
      <Accordion type="multiple">
        <AccordionItem value="recent-sessions">
          ...
          <AccordionContent>
            <CompanionList title="Recent Sessions" companions={recentSessions} />
          </AccordionContent>
        </AccordionItem>
        <AccordionItem value="created-companions">
          <AccordionTrigger className="text-2xl font-bold">Companions Created</AccordionTrigger>
          <AccordionContent>
            <CompanionList title="Your Companions" companions={companionCreated} />
          </AccordionContent>
        </AccordionItem>
      </Accordion>
    </main>
